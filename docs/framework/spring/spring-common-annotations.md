...
> @Bean 作用于方法上，表明该方法返回一个bean对象，默认是单例
>
> @Component等 作用于类上
...
👉 需要注意的是：**一个请求方法只可以有一个`@RequestBody`，但是可以有多个`@RequestParam`和`@PathVariable`**。 如果方法必须要用两个 `@RequestBody`来接受数据的话，大概率是你的数据库设计或者系统设计出问题了！
