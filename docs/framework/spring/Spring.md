...
https://mp.weixin.qq.com/s/h_6h9Q63eEcAjeRwIA8Rlg
...
二级缓存：存放实例化的bean对象（**尚末填充属性**的半成品），用于解决循环依赖
...
![图片](https://cdn.jsdelivr.net/gh/YiENx1205/cloudimgs/notes/202302211652205.png)



## 为什么要有三级缓存

先说“一级缓存”的作用，变量命名为 singletonObjects，结构是 Map<String, Object>，它就是一个单例池，将初始化好的对象放到里面，给其它线程使用，**如果没有第一级缓存，程序不能保证 Spring 的单例属性。**

看“三级缓存”的作用，变量命名为 singletonFactories，结构是 Map<String, ObjectFactory<?>>，Map 的 Value 是一个对象的代理工厂，所以“三级缓存”的作用，其实就是用来存放对象的代理工厂。

**主要作用是存放半成品的单例 Bean，目的是为了“打破循环”**
...
>
> **通过 A 的工厂的代理对象，生成了两个不同的对象 A1 和 A2**，所以为了避免这种问题的出现，搞个二级缓存，把 A1 存下来，下次再获取时，直接从二级缓存获取，无需再生成新的代理对象。
>
> **所以“二级缓存”的目的是为了避免因为 AOP 创建多个对象，其中存储的是半成品的 AOP 的单例 bean。**
>
> 如果没有 AOP 的话，只要 1、3 级缓存，就可以满足要求。
