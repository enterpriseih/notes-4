...
<img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/2020-8/Kernel_Layout.png" alt="Kernel_Layout" style="zoom:67%;" />
...
# 报错 netstat: option requires an argument -- p 
```
...
- **`netstat -anp |grep 端口号`**：查看端口占用情况；-a,显示所有 -n,不用别名显示,只用数字显示 -p,显示进程号和进程名
...
[参考](https://blog.51cto.com/u_15067249/3445082)
...
编辑指令如果包含多条语句时，可以用分号分隔，
处理文本时，若未指定分隔符，则默认将空格、制表符等作为分隔符。
print是最常见的指令。

FS保存或设置字段分隔符, 如FS=':'
$n指定分隔的第n个字段, 如$1、$3分别表示第1、3列
$0当前读入的整行文本内容
...
FNR当前行在原文件中的行号. 
	awk '{print "第" FNR "行", "有" NF "列"}' test.txt

-F参数: 指定分隔符，可指定一个或多个，可省略，默认空格或Tab
		指定多个分隔符，加[]
		[ :]+，+表示一个或多个，这里表示一个或多个空格或冒号
