...
### 1. [n以内的素数](https://leetcode-cn.com/problems/count-primes/description/)

埃氏筛法在每次找到一个素数时，将能被素数整除的数排除掉。

![Sieve_of_Eratosthenes_animation.gif](https://cdn.jsdelivr.net/gh/YiENx1205/cloudimgs/code/202209081123570.gif)
...
将`a`除以`b`得到余数`r`，`a`和`b`的最大公约数即为`b`和`r`的最大公约数。

辗转相除法，直到余数为0，此时的除数是最大公约数
